...
const styles = ['numbers','straight','quadvertex','quadline','arc','altarc','colours'];
  ...
  try {
    ...
    for (let i in data) {
      ...
      styles.forEach( style => {
        ...
        let len = 0
        if( style !== 'numbers' && style !== 'colours' ) {
          const re = /d='([\w|\s|\d|,]+)'/;
  ...
  styles.forEach( style => {
    if (style != 'numbers' && style != 'colours') {
      ...
      lengths.forEach( len => {
        ...
        const similarIDs = matches.map(m => m._id)
      ...
      index.forEach( idx => {
        ...
        // remove self
        idx[style]['shared'] = output[l].filter(o => o !== idx._id)
      })
    }
  })
