...
async function source(n) {
  ...
  await generateSource(`${n}`)
  await couch.populateDB(`./data/source${n}.json`,`source${n}`)
  await couch.createMetaDocs( `source${n}`,`${n}`)
}

async function index(n) {
  console.log( '-------------- INDEX SETUP -----------------' )
  await generateIndex(`${n}`)
  await couch.populateDB(`./data/index${n}.json`,`index${n}`)
  await couch.createMetaDocs( `index${n}`,`${n}`)
  console.log( '------------------- END ---------------------' )
}
...
async function generateSource(order) {
  try {
    const readFile = util.promisify(fs.readFile);
    const sourceData = await readFile(`./data/source${order}.json`)
    const data = JSON.parse(sourceData)
    let finalJSON = []
    for (let i in data) {
      finalJSON.push({ "_id": `${parseInt(i)+1}`, "numbers": data[i] })
    }
    const finalString = JSON.stringify(finalJSON)
    await saveFile(`data/source${order}DB.json`, finalString)
  } catch (error) { console.log( 'generateSource:', error ) }
}



async function generateIndex(order) {
  let output = '['
  let lengths = {}

  const readFile = util.promisify(fs.readFile);
  const sourceData = await readFile(`./data/source${order}DB.json`)
  const data = JSON.parse(sourceData)
  ...
  for (let i in data) {
    const valuesArray = data[i].numbers
  ...
  const final = parseInt(order) === 4 ? withSims : withShared
