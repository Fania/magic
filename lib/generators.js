...
async function source(order) {
  try {
    ...
    const sourceData = await fs.readFileSync(`./data/source${order}.json`)
    const data = JSON.parse(sourceData)
    let finalJSON = []
    for (let i in data) {
      finalJSON.push({ "id": `${parseInt(i)+1}`, "numbers": data[i] })
    }
    // Agrippa ONLY
    // for (let [key, value] of Object.entries(data)) {
    //   finalJSON.push({ "id": key, "numbers": value })
    // }
    await couch.populateDB(finalJSON,`source${order}`)
...
async function index(order) {
  try {
    ...
    await couch.populateDB(finalJSON,`index${order}`)
