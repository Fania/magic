...
async function getSourceChanges(order, remoteData, localData) {
  try {
    ...
    } else { console.log('This should never happen! As in, the remote and local are not identical but there are also no changes in the remote...') }
  } catch (error) { console.log( 'getChanges:', error ) }
  finally { console.log( 'Changes successfully downloaded.' ) }
}
