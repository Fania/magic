...
async function viewSourceDB(order, offset=0) {
  console.log('viewSourceDB', order, offset)
  try {
...
async function populateDBSource(data, order) {
  try {
    ...
    data.push({
      ...
      "views": {
        "i": {
          "map": "function (doc) { emit(doc.numbers, 1); }"
        }
      }
    })
    let output = data
  ...
  } catch (error) { console.log( 'populateDBLive:', error ) }
  finally {
    console.log( '-------------------- END ---------------------' )
  }
}
...
async function areThereChangesSource(order) {
  try {
    // REMOTE STUFF
    const remote = await nano.db.changes(`source${order}`, {include_docs: true})
    const remoteObjects = remote.results.filter(res => ! res.id.includes('_design/'))
    const remoteData = remoteObjects.map(r => r.doc.numbers.array).sort()
    // LOCAL STUFF
    const localFile = fs.readFileSync(`./data/source${order}.json`)
