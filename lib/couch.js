...
function getReduceDocs() {
  const reducer = {
    "_id": "_design/reducer",
    "views": {
      "sharedLengths": {
        "reduce": "function (keys, values, rereduce) { let result = []; for(i=0; i < values.length; i++) { if(rereduce) { result[i] = values[i]; } else { result = values; } } return(result); }",
