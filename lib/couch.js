...
async function createMetaDocs(dbName, order) {
  try {
    const db = await getDB(dbName)
    ...
    await db.get('_design/order', async (error, existing) => { 
      ...
      console.log( 'Upserted numeric order design doc.' )
    })
    // console.log(  )
    
    // await db.insert(numericOrder)
    ...
    if (parseInt(order) === 4) {
      ...
      await db.get('_design/filter', async (error, existing) => { 
        ...
        console.log( 'Upserted unique filter design doc.' )
      })
    }
