...
async function statusReport() {
  try {
    ...
    for (let o = 0; o < orders.length; o++) {
      ...
      // LOCAL STUFF
      const localFile = fs.readFileSync(`./data/source${orders[o]}.json`)
      ...
      if (JSON.stringify(remoteData) !== JSON.stringify(localData)) {
        ...
        if (diffRL.length > 0) {
          ...
          data.sort()
          fs.writeFileSync(`./data/source${orders[o]}.json`, JSON.stringify(data))
        } else { 
          log(`locally\n`)
...
async function createIFNeeded(order) {
  ...
  if (!info.includes(`source${order}`)) { 
    ...
    fs.writeFileSync(`./data/source${order}.json`, "[]")
  }
}
