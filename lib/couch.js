...
async function request(endpoint, method='GET', data={}) {
  try {
    ...
    console.log('Couch response:',response.status,response.statusText)
    return response.data
  } catch (error) { 
    console.error('Axios request error:',error.response.status,error.response.statusText)
...
async function insertTheme(theme) {
  try {
    ...
    console.log(exists)
    if (exists.error && exists.error == 'not_found') {
