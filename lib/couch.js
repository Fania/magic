...
async function viewDB(db, style, offset=0) {
  // console.log('viewDB', order, style, filter, offset)
  try {
    // u for unique 383, i for inclusive 880
    const response = await request(`${db}/_design/d/_view/${style === 'unique' ? 'u' : 'i'}?include_docs=false&skip=${offset}`)
    return response.data
...
async function updateCuratedCounterDB() {
  try {
    const requestURL = `curated/621e1e19efb4ec48e16d5254c21be53a`
    ...
    const confirmation = await request(requestURL,'PUT',tmpItem,tmpHeaders)
...
// TODO finish converting axios request
async function populateDBSourceSuzuki(data, order) {
  try {
    console.log( `(Re-) Creating database for suzuki` )
    // bulk docs needs individual rev ids for updates
    const info = await request(`_all_dbs`)
    if (info.data.includes(`suzuki`)) { 
      await request(`suzuki`,'DELETE')
    }
    await request(`suzuki`,'PUT')
    ...
    let output = data
    // bulk docs needs individual rev ids for updates
    await request(`suzuki/_bulk_docs`,'POST',{docs:output})
  } catch (error) { console.log( 'populateDBSourceSuzuki:', error ) }
  finally {
    console.log( '-------------------- END ---------------------' )
  }
}
