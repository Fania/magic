...
function createCirclesSVGs(order,coordsObject,id,classes) {
  ...
  for (let a in coordsObject) {
    ...
    blobs += `<circle cx='${coordsObject[a][0] * 100}' cy='${coordsObject[a][1] * 100}' r='40' fill='#${color}' stroke='#${color}' />`;
  }
  return `<svg id='circles-${order}-${id}' class='${classes}' viewBox='-40 -40 380 380'>${blobs}</svg>`;
}
