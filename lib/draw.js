...
function prepareSVG(order, style, coordsObject, id) {
  ...
  switch(style) {
    ...
    case "colours":
      return createNumColoursSVGs(order, coordsObject, id);
    default:
      return createQuadraticCurveVertices(order, coordsObject, id);
  }
}
...
function createNumColoursSVGs(order, coordsObject, id) {
  // console.log(`preparing number matrix svg for square ${counter}`);
  let blobs;
  let w = parseInt(order) * sizeInc;
  const colours = ['091540','7692FF','ABD2FA','3D518C','1B2CC1',
                   '012A36','00A6FB','1F7A8C','50C9CE','58355E',
                   'B388EB','231942','FF9FE5','5E548E','419D78','654597']
  for (let a in coordsObject) {
    // console.log(a)
    blobs += `<circle fill='#${colours[parseInt(a)-1]}' cx='${coordsObject[a][0] * 100}' cy='${coordsObject[a][1] * 100}' r='40'/>`;
  }
  // 0 -50 380 370 for order 4
  return `<svg id='colours-${order}-${id}' class='order-xt pad' viewBox='-40 -40 380 380'>${blobs}</svg>`;
}
