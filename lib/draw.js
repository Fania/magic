...
function prepareSVG(order, style, coordsObject, id) {
  ...
  switch(style) {
    ...
    case "circles":
      return createCirclesSVGs(order, coordsObject, id);
    case "blocks":
      return createBlocksSVGs(order, coordsObject, id);
    case "tetromino":
      return createTetrominoSVGs(order, coordsObject, id);
    default:
      return createQuadraticCurveVertices(order, coordsObject, id);
  }
}
...
function createCirclesSVGs(order, coordsObject, id) {
  // console.log(`preparing number matrix svg for square ${counter}`);
  let blobs;
  let w = parseInt(order) * sizeInc;
  for (let a in coordsObject) {
  ...
  return `<svg id='circles-${order}-${id}' class='order-xt pad' viewBox='-40 -40 380 380'>${blobs}</svg>`;
}

function createTetrominoSVGs(order, coordsObject, id) {
  // console.log(`preparing number matrix svg for square ${counter}`);
  let blobs
  let w = parseInt(order) * sizeInc
  // console.log(w)
  ...
  return `<svg id='tetromino-${order}-${id}' class='order-xt pad' viewbox='${-50} ${-50} ${w+50} ${w+50}'>${blobs}</svg>`;
}


function createBlocksSVGs(order, coordsObject, id) {
  // console.log(`preparing number matrix svg for square ${counter}`);
  let blobs
  let w = parseInt(order) * sizeInc
  // console.log(w)
  ...
  // return `<svg id='colours-${order}-${id}' class='order-xt pad' viewbox='${-200} ${-170} ${w-sizeInc+380} ${w-sizeInc+380}'>${blobs}</svg>`;
  return `<svg id='blocks-${order}-${id}' class='order-xt pad' viewbox='${0} ${0} ${w} ${w}'>${blobs}</svg>`;
}
